...
object RNG {
  ...
  def uniform(a: Double = 0.0, b: Double = 1.0) = for {
    x <- nextDouble
  } yield a + (b - a) * x
  ...
  def gaussian(mean: Double = 0.0, variance: Double = 1.0): Rand[Double] = State { rng =>
    ...
    def update(x: Double, y: Double, i: Int) = { (rng: RNG) =>
      if (i < 127) {
        val y0 = ytab(i)
        val y1 = ytab(i + 1)
        val (rng1, uni) = uniform()(rng)
        (rng1, (x, y1 + (y0 - y1) * uni))
      } else {
        val (rng1, (a, b)) = uniformPair(rng)
    ...
    def inner(y: Double): StateT[Trampoline, RNG, Double] = StateT { rng =>
      ...
      if (j >= ktab(i)) {
        ...
        if (y1 < math.exp(-0.5 * x1 * x1)) {
          Return((rng2, if (sign != 0) x1 else -x1))
        } else Suspend(() => inner(y1).run(rng2))
      } else Return((rng1, if (sign != 0) x else -x))
    }

    inner(0).run(rng).run
  }

  def cauchy(l: Double = 0.0, s: Double = 1.0) = for {
    r <- nextDouble
  } yield l + s * math.tan(math.Pi * (r - 0.5))

  def exponential(b0: Option[Double @@ Positive] = positive(1.0)) = { // b0 > 0.0
    val b = b0.getOrElse(sys.error("Value provided to exponential: expected positive value but none found."))
    for {
      r <- nextDouble
    } yield -math.log(1 - r) / b
  }

  def gamma(b0: Int = 2, b1: Double = 2.0) = {
    val exp = exponential(positive(1.0 / b1))
    for {
      x <- sequence(b0)(exp)
    } yield x.sum
  }
  ...
  def doubles(n: Int) = nextDouble replicateM n

  def randDouble(x: Double) = for {
    r <- nextDouble
  } yield x * r
